        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            ...
            <div className="flex items-center gap-4">
               {/* Language Switcher Desktop - Grid Layout for Stability */}
               <div className="grid grid-cols-3 gap-1 bg-slate-100 dark:bg-white/5 rounded-lg p-1 border border-slate-200 dark:border-white/10">
                 <button onClick={() => setLanguage('ar')} className={`w-9 h-7 flex items-center justify-center text-xs font-bold rounded-md transition-all ${language === 'ar' ? 'bg-white dark:bg-white/20 shadow-sm text-primary' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400'}`}>AR</button>
                 <button onClick={() => setLanguage('en')} className={`w-9 h-7 flex items-center justify-center text-xs font-bold rounded-md transition-all ${language === 'en' ? 'bg-white dark:bg-white/20 shadow-sm text-primary' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400'}`}>EN</button>
                 <button onClick={() => setLanguage('fr')} className={`w-9 h-7 flex items-center justify-center text-xs font-bold rounded-md transition-all ${language === 'fr' ? 'bg-white dark:bg-white/20 shadow-sm text-primary' : 'text-slate-500 hover:text-slate-800 dark:text-slate-400'}`}>FR</button>
               </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-2xl pt-24 px-6 animate-fade-in h-screen overflow-y-auto transition-colors duration-300">
          <div className="flex flex-col gap-4 max-w-sm mx-auto">
            ...
            {/* Language Switcher Mobile - Grid Layout */}
             <div className="flex flex-col gap-3 bg-slate-100 dark:bg-white/5 p-4 rounded-2xl mb-2 border border-slate-200 dark:border-white/10">
              ...
              <div className="grid grid-cols-3 gap-2">
                 <button onClick={() => setLanguage('ar')} className={`flex items-center justify-center px-3 py-2 rounded-lg font-bold text-sm ${language === 'ar' ? 'bg-primary text-white' : 'bg-white dark:bg-white/10 text-slate-500 dark:text-slate-400'}`}>العربية</button>
                 <button onClick={() => setLanguage('en')} className={`flex items-center justify-center px-3 py-2 rounded-lg font-bold text-sm ${language === 'en' ? 'bg-primary text-white' : 'bg-white dark:bg-white/10 text-slate-500 dark:text-slate-400'}`}>English</button>
                 <button onClick={() => setLanguage('fr')} className={`flex items-center justify-center px-3 py-2 rounded-lg font-bold text-sm ${language === 'fr' ? 'bg-primary text-white' : 'bg-white dark:bg-white/10 text-slate-500 dark:text-slate-400'}`}>Français</button>
              </div>
            </div>
